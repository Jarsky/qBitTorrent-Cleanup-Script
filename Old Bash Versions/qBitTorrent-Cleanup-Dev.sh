...
function helpCMD(){
        ...
        -run                        Will use the new qBitTorrent CLI mode
        -run test                   Will run checks in Read-Only mode
...
if [[ $1 == "-h" || $1 == "--help" ]]; then
        ...
        # #parse json file and cross check qBitTorrent with files in downloads folder
        # #torrentNames=($(jq '.[].name' $jsonFilename))
        ...
        #                         if [ ! -d "$torrentPath/$torrentName" ]; then
        #                                 echo -e "$(dateFormat) ${WARN} $torrentName is not a Directory, ignoring."  | tee -a "$logFile"
        #                         else
        #                                 echo -e "$(dateFormat) ${INFO} $torrentPath/$torrentName exists"  | tee -a "$logFile"
        #                         fi
        #                 else
        #                 echo -e "$(dateFormat) ${ERROR} $torrentPath/$torrentName doesn't exist"  | tee -a "$logFile"
        #                 fi
        #         done
        #         echo -e "$(dateFormat) ${INFO} Check has finished"
        #         #rm -f $jsonFilename
fi