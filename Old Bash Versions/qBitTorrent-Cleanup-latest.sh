...
function buildArrays() {
        ...
        #old qBTappLog=`cat $qBitTorrentLog | grep 'Error: "Directory not empty"' | awk '{ gsub(/"|\.$/,"",$13); print $13 }'`
        qBTappLog=$(grep 'Removed torrent but failed to delete its content and/or partfile' "$qBitTorrentLog" | awk -F 'Torrent: "|"' '{print $2}')
